...
import { Search, Filter, Instagram, Twitter, Youtube, BookText as TikTok, Star, ChevronDown } from 'lucide-react';
...
const FindInfluencersPage: React.FC = () => {
  ...
  const allNiches = Array.from(
    new Set(
      mockInfluencerProfiles.flatMap(influencer => influencer.niche)
    )
  ).sort();
  ...
  const filteredInfluencers = mockInfluencerProfiles.filter(influencer => {
    ...
    const matchesAudienceMin = audienceMinSize === '' || influencer.audienceSize >= audienceMinSize;
    const matchesAudienceMax = audienceMaxSize === '' || influencer.audienceSize <= audienceMaxSize;
    
    // Filter by engagement rate
    const matchesEngagementRate = minEngagementRate === '' || influencer.engagementRate >= minEngagementRate;
    
    return matchesSearch && matchesNiche && matchesAudienceMin && matchesAudienceMax && matchesEngagementRate;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Discover influencers who match your brand and audience.</p>
      </div>
          ...
          <Card>
            ...
            <CardContent className="p-6">
              <div className="space-y-6">
                {/* Niche Filter */}
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Niche</h3>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Audience Size</h3>
                  <div className="flex items-center space-x-2">
                    <Input
                      type="number"
                      placeholder="Min"
                      value={audienceMinSize}
                      onChange={(e) => setAudienceMinSize(e.target.value ? parseInt(e.target.value) : '')}
                      className="w-full"
                    />
                    ...
                    <Input
                      ...
                      onChange={(e) => setAudienceMaxSize(e.target.value ? parseInt(e.target.value) : '')}
                      className="w-full"
                    />
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Engagement Rate</h3>
                  <div className="flex items-center space-x-2">
                    <Input
                      type="number"
                      placeholder="Min %"
                      value={minEngagementRate}
                      onChange={(e) => setMinEngagementRate(e.target.value ? parseFloat(e.target.value) : '')}
                      className="w-full"
                    />
                    <span className="text-gray-500">% +</span>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row">
                    <div className="md:w-1/4 bg-gradient-to-br from-[#2A0A5E] to-[#3A1A7E] p-6 flex flex-col items-center justify-center text-white">
                      ...
                      <div className="flex space-x-2 mb-4">
                        {influencer.socialMedia.slice(0, 2).map((social, index) => (
                          <div key={index} className="flex items-center">
                            {getSocialIcon(social.platform)}
                            <span className="ml-1 text-sm">{social.followers >= 1000000 
                      ...
                      <div className="flex items-center">
                        <Star size={16} className="text-[#00F5FF] fill-[#00F5FF]" />
                        <span className="ml-1 text-sm">{influencer.engagementRate}% Engagement</span>
                      </div>
                    </div>
                    <div className="md:w-3/4 p-6">
                      ...
                      <p className="text-gray-700 mb-6 line-clamp-3">{influencer.bio}</p>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div className="text-center p-3 bg-gray-50 rounded-md">
                          ...
                          <p className="font-semibold">${influencer.pricing?.postRate?.toLocaleString() || 'N/A'}</p>
                        </div>
                        <div className="text-center p-3 bg-gray-50 rounded-md">
                          <p className="text-sm text-gray-500">Top Platform</p>
                          <p className="font-semibold">
                            {influencer.socialMedia.sort((a, b) => b.followers - a.followers)[0].platform}
                          </p>
                        </div>
                      </div>
